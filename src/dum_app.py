...
import asyncio
from prefinal import locations
import logging
import random
...
prefinal_file_path = os.path.join(os.getcwd(), "prefinal_v3.csv")
...
st.title("Async Function in Streamlit")

# Create two buttons
start_button = st.button("Start Async Function")
stop_button = st.button("Stop Async Function")

# Create a placeholder for the async task and stop event
...
# Function to start the async function
...
# Function to stop the async function
...
if start_button:
    ...
    loop.create_task(start_async_function())
    loop.run_forever()
elif stop_button:
    stop_async_function()
